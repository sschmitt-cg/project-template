...
1. Check whether a `chore/sync-template` branch already exists locally or remotely:
   - Run `git branch --list chore/sync-template` (local)
   - Run `git branch -r --list origin/chore/sync-template` (remote)
   If it exists, check it out and reset it to main (`git checkout chore/sync-template` then `git reset --hard main`). If it does not exist, create it fresh.
2. Stage the changed files
3. Commit with the message: `chore: sync from project-template`
4. Push the branch (use `--force-with-lease` if resetting an existing branch)
